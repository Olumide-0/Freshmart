...
export default function GroceriesTeaserDetailPage({ params }) {
  const { slug } = use(params);
  const product = GROCERIES_TEASER.find((p) => p.slug === slug);
  ...
  const total = (42 * quantity).toFixed(0);
  ...
  return (
    <div className="w-full bg-[#F6F0E3] px-[120px] py-[24px]">
      <div className="flex items-center gap-2 text-[13px] text-gray-500">
        ...
        <span className="font-semibold text-[#C6672E]">{product.title}</span>
      </div>
      ...
      <div className="mt-[24px] grid grid-cols-2 gap-[24px]">
        <div>
          ...
          <div className="mt-[16px] grid grid-cols-4 gap-[12px]">
            {thumbnails.map((img, i) => (
              <div key={i} className="relative aspect-square overflow-hidden rounded-[10px] bg-white">
                <Image src={img} alt={`${product.title} thumbnail ${i + 1}`} fill className="object-cover" />
              </div>
            ))}
          </div>
          ...
          <div className="mt-[24px] rounded-[16px] bg-white p-[24px]">
            ...
            <p className="mt-[10px] text-[14px] leading-[1.6] text-gray-600">
              {product.subtitle}. Handpicked at peak ripeness from trusted local farms
        ...
        <div className="rounded-[16px] bg-white p-[32px]">
          <h1 className="text-[28px] font-extrabold text-[#1F2937]">{product.title}</h1>
          ...
          <div className="mt-[24px] flex items-center justify-between border-t border-gray-100 pt-[20px]">
            ...
            <span className="text-[14px] font-semibold text-[#C6672E]">$42.00/ltr</span>
          </div>
          ...
          <div className="mt-[20px] flex items-center justify-between rounded-[14px] bg-[#F6F0E3] p-[20px]">
            ...
            <div className="text-right">
              ...
              <p className="text-[12px] text-gray-500">$42.00/g</p>
            </div>
          </div>
