...
export default function FreshGroceryDetailPage({ params }) {
  const { slug } = use(params);
  const product = FRESH_GROCERIES.find((p) => p.slug === slug);
  ...
  if (!product) {
    return (
      <div className="w-full bg-[#F6F0E3] px-4 py-6 sm:px-8 md:px-12  xl:py-[48px]">
  ...
  return (
    <div className="w-full bg-[#F6F0E3] px-4 py-5 sm:px-8 md:px-12 xl:py-6">
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-gray-500 sm:gap-2 sm:text-[13px]">
        ...
        <Link href="/fresh-groceries" className="hover:text-gray-700">
          Fresh groceries
        </Link>
