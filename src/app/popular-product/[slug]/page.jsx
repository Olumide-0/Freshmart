...
export default function PopularProductDetailPage({ params }) {
  const { slug } = use(params);
  const product = POPULAR.find((p) => p.slug === slug);
  ...
  const total = (numericPrice * quantity).toFixed(0);
  ...
  return (
    <div className="w-full bg-[#F6F0E3] px-4 py-5 sm:px-8 md:px-12  xl:py-6">
      ...
      <div className="flex flex-wrap items-center gap-1.5 text-[11px] text-gray-500 sm:gap-2 sm:text-[13px]">
        ...
        <span className="max-w-[200px] truncate font-semibold text-[#C6672E] sm:max-w-none">
          {product.name}
        </span>
      </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-5 sm:mt-6 sm:gap-6 lg:grid-cols-2 lg:gap-6">
        ...
        <div className="min-w-0 rounded-[14px] bg-white p-4 sm:rounded-[16px] sm:p-5 lg:p-6 xl:p-8">
          {/* Product Name */}
          <h1 className="break-words text-[22px] font-extrabold text-[#1F2937] sm:text-[25px] lg:text-[28px]">
            {product.name}
          </h1>
